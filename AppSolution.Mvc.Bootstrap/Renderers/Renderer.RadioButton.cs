...
namespace AppSolution.Mvc.Bootstrap.Renderers
{
    internal static partial class Renderer
    {
        public static string RenderRadioButton(HtmlHelper html, BootstrapRadioButtonModel model)
        {
            ...
            checkBoxDiv.MergeAttribute("class", "radio");
            ...
            if (!string.IsNullOrEmpty(model.name))
                input.Attributes.Add("name", model.name);
            ...
            span.InnerHtml = !string.IsNullOrEmpty(model.text) ? model.text : model.metadata.DisplayName;

            label.InnerHtml = input.ToString(TagRenderMode.SelfClosing) + span.ToString();
